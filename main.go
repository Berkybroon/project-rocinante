...
func renderHTML(root *treeNode, rootPath string, totalFiles int, totalBytes int64) string {
	...
	b.WriteString("<div class=\"warning\"><strong>Warning:</strong> Very large filesystems may take a while to render in the browser. If the page feels slow, try scanning a smaller target folder, or wait a bit.</div>")
	b.WriteString(fmt.Sprintf("<div class=\"meta\"><strong>Root:</strong> %s<br><strong>Files:</strong> %d<br><strong>Total size:</strong> %s</div>", html.EscapeString(rootPath), totalFiles, html.EscapeString(humanSize(totalBytes))))
	b.WriteString("<ul class=\"tree\">")
